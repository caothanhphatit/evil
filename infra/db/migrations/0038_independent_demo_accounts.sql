CREATE OR REPLACE FUNCTION seed_full_demo_account_stock(target_player UUID)
RETURNS VOID
LANGUAGE plpgsql
AS $$
BEGIN
    ...
    WHERE town.player_token = target_player
    ...
    WHERE ruleset = 'demo_full_stock_v1'
      AND town_id IN (SELECT town_id FROM town WHERE player_token = target_player);
    ...
    SELECT target_player, 1, product.release_id, product.product_id, 999
    FROM economy_product_definition product
    JOIN player_hunter hunter
      ON hunter.player_token = target_player
...
SET player_token = CASE normalized_email
        WHEN 'demo2@evil.local' THEN '00000000-0000-4000-8000-00000000a002'::UUID
        WHEN 'demo3@evil.local' THEN '00000000-0000-4000-8000-00000000a003'::UUID
        ELSE player_token
    END,
    updated_at = now()
