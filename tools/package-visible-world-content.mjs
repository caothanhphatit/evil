...
const villageRoot = path.join(repoRoot, "game-assets/extracted/exported/village");
...
async function publish(sourcePath, outputPath) {
  ...
  return { sourcePath, publicPath: `/content/releases/visible-world-v1/${outputPath}`, bytes: indexed.bytes, sha256: indexed.sha256 };
}
...
async function publishLocal(sourcePath, outputPath) {
  ...
  await fs.mkdir(path.dirname(output), { recursive: true });
  await fs.writeFile(output, payload);
  return { sourcePath, publicPath: `/content/releases/visible-world-v1/${outputPath}`, bytes: payload.length, sha256: createHash("sha256").update(payload).digest("hex") };
}
...
const fieldMap = await publish("textures/map_new01__185.png", "maps/map_new01.png");
const tileBindings = [
...
const villageTiles = await Promise.all(tileBindings.map(async ([file, x, y]) => ({
  ...await publish(`sprites/${file}`, `village/background/${file}`), x, y, z: 499,
})));
...
const foreground = await Promise.all(foregroundBindings.map(async ([id, x, y, z]) => ({
  ...await publishLocal(`game-assets/extracted/exported/village/foreground/${id}.png`, `village/foreground/${id}.png`), id, x, y, z,
})));
...
const npcs = Object.fromEntries(await Promise.all(Object.entries(extracted.npcs).map(async ([role, frames]) => [role, {
  ...
  frames: await Promise.all(frames.map(async (frame) => ({ ...await publishLocal(frame.file, `village/npcs/${role}/${frame.name}.png`), frame: frame.frame, name: frame.name }))),
}])));
...
const village = { tiles: villageTiles, foreground, decorations, npcs, bindingState: "confirmed-scene-bindings" };
const manifest = { schemaVersion: 2, releaseId: "visible-world-v1", bindingState: "migration-derived-visible-world", map: fieldMap, fieldMap, village, actors };
const serializedManifest = `${JSON.stringify(manifest, null, 2)}\n`;
// Keep the browser-facing filename distinct from Vite's reserved manifest route.
...
console.log(`Packaged ${actors.length} original Spine families and one map for the visible world.`);
