...
describe("Hunter Info modal shell", () => {
  ...
  it("renders the recovered Hunter actor and all eight confirmed equipment positions", async () => {
    ...
    const paperDoll = await readFile(resolve(repositoryRoot, "apps/web/src/game/hunter-paper-doll.ts"), "utf8");
    ...
    expect(paperDoll).toContain('setAnimation(0, "hunter_stay", true)');
    expect(actor).toContain("height * 0.62 / bounds.height");
    expect(actor).toContain("1.9,");
    expect(actor).toContain("prepareHunterPaperDoll(spine");
    expect(actor).not.toContain("hunterPaperDollVisual");
    expect(actor).not.toContain("applyHunterSpineSkin");
    expect(styles).toContain(".hunter-paper-doll img { width: 90%; height: 90%");
    expect(rosterActors).toContain("avatarBounds.height * 0.82");
    expect(rosterActors).toContain("prepareHunterPaperDoll(spine");
    expect(rosterActors).not.toContain("applyHunterSpineSkin");
  ...
  it("keeps the desktop roster compact and scrollable beyond two rows", async () => {
    ...
    const preCanonicalRoster = styles.slice(0, styles.indexOf("/* Canonical Hunter roster frame"));
    expect(preCanonicalRoster).not.toContain(".hunter-roster-panel");
    expect(preCanonicalRoster).not.toContain(".hunter-roster-card");
