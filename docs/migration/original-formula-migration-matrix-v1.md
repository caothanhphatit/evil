...
| Monster outgoing damage | Exact catalog value is preserved on the monster snapshot, but live attack input is divided by temporary `250` before the recovered neutral Hunter-intake tail | Original damage catalog, RandDamage placement, feel-based armor bands, default `0.75` factor, minimum damage, dodge, first-shield routing and HP floor are connected; the native selected runtime factor applied to catalog damage is still unresolved | Remove the compatibility divisor only after the selected-factor writer and representative live input/output vectors are recovered; do not tune it from the demo roster |
...
| Hunter armor/defense | Captured runtime armor, or the explicit fixture defense fallback, feeds the recovered feel-band armor scratch; unresolved optional pre-armor modifiers stay disabled | Feel bands/factors, 32 ordered pre-armor mutations, two-stage armor scratch, exact constant final factor `0.75`, minimum branch, first-shield absorption/spillover and HP floor are recovered | Bind the remaining optional modifier writers and multi-shield state before claiming the complete original intake formula |
| Hunter dodge/evasion | Profile `CalcDodge` plus connected skill bonus is rolled before armor/HP; riding-pet dodge and effect type 5 remain disabled | Pass 18 recovers normal `CalcDodge` production/consumption, signed exclusive rolls, Meze bypass, pet fallback, and the `HunterCtrl.Damaged` early exit; direct Evil effect-54 remains a separate gate | Bind the missing named sources and Unity PRNG sequencing before claiming exact original outcome sequences |
| Damage variance | The persisted exact 30-value `RandDamage` stream is applied in both connected neutral ordinary-attack directions | Exact wrapping stream and its placement in the connected neutral tails are recovered | Resolve Unity/global sequence interaction and unclassified skill/alternate-mode callers before extending the same stream contract beyond neutral ordinary attacks |
